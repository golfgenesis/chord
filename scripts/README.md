...
ใช้ **โมเดล vision บน Ollama ในเครื่อง** (รันบน Mac, ไม่มี API key, ไม่มี limit) อ่านรูปแผ่นคอร์ด → เขียนเป็น **Inline ChordPro**
...
ติดตั้ง [Ollama](https://ollama.com/download) แล้ว `ollama pull qwen2.5vl:7b` (รุ่น default — หรือรุ่น vision อื่นแล้วใส่ `--model` / ตั้ง env `OLLAMA_MODEL`)
ยิงตรงไป `http://127.0.0.1:11434` (เปลี่ยนได้ด้วย env `OLLAMA_HOST` / `OLLAMA_MODEL`) — สคริปต์มี preflight เช็คว่า server + model พร้อมไหม ถ้าไม่พร้อมจะบอกคำสั่ง `ollama pull` ให้
> Ollama อ่าน **WebP ไม่ได้** → สคริปต์แปลงเป็น PNG ด้วย `sips` ของ macOS ก่อนส่งให้อัตโนมัติ (ไม่ต้องลง dependency เพิ่ม)

| คำสั่ง | ทำอะไร | resume/หยุด |
|---|---|---|
| `npm run chordpro:backfill` | **⭐ extract เพลงที่ยังไม่มี** → `data/songs-md/<id>.md` (รันรัวๆ ไม่มีดีเลย์ เร็วเท่าที่ GPU/unified memory ไหว) | **กด Ctrl+C หยุดได้**, รันซ้ำ = ทำต่อ (ข้ามไฟล์ที่มีแล้ว) |
...
| `npm run chordpro:fix -- <id>` | แก้รายเพลง: re-extract + อัป R2 ทับ + rebuild ในคำสั่งเดียว | — |

ตัวเลือก backfill (ใส่หลัง `--`):
`--limit 50` (ลองสั้นๆ) · `--start 70570` (เฉพาะ id ตั้งแต่นี้ขึ้นไป) · `--ids 11,19,42` (เฉพาะ id ที่ระบุ) · `--force` (ทำใหม่แม้มีไฟล์แล้ว) · `--model <name>` (เปลี่ยนรุ่น Ollama) · `--concurrency 2` (เฉพาะตอนตั้ง `OLLAMA_NUM_PARALLEL>1`)
...
node scripts/local-backfill.mjs --ids 19 --force
npm run chordpro:upload
npm run data
npm run dev                      # แล้ว Ctrl+Shift+R
# หรือสั้นๆ ทำทั้ง 3 ขั้นในคำสั่งเดียว:
npm run chordpro:fix -- 19
```
> ถ้าพังเป็น "แบบแผน" หลายเพลงเหมือนกัน → ปรับ **prompt** (`SYSTEM_INSTRUCTION`) ใน `scripts/local-backfill.mjs` แล้ว `--force` ใหม่ทั้งกลุ่ม
