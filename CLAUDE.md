...
# ChordPro text pipeline (chord-sheet images → inline ChordPro markdown via local Ollama) — see below
npm run chordpro:backfill   # ⭐ local Ollama vision model → data/songs-md/<id>.md (resumable, no delay, skips cached)
...
- `scripts/local-backfill.mjs` — Node. The local-Ollama ChordPro backfill (see below). `scripts/upload_md_r2.py` — pushes `data/songs-md/*.md` → R2 `md/<id>.md`.
...
## ChordPro text pipeline (chord-sheet images → inline ChordPro markdown, via local Ollama)
...
- **Extraction** ([scripts/local-backfill.mjs](scripts/local-backfill.mjs), `npm run chordpro:backfill`) — sends each WebP chord-sheet image (local `images/<name>.webp` first, else R2) to a **local Ollama vision model** (default `qwen2.5vl:7b`; override with `--model` or `OLLAMA_MODEL`) over HTTP at `http://127.0.0.1:11434/api/generate` (override `OLLAMA_HOST`) and writes the returned Inline ChordPro to `data/songs-md/<id>.md`. **100% local — no API key, no rate limit, no per-image delay, no circuit breaker**; it loops back-to-back as fast as Apple-Silicon unified memory computes (`--concurrency` default 1 because a single Ollama model serializes on the GPU anyway). **Ollama can't decode WebP** (it 400s with "Failed to load image"), so each image is transcoded to PNG with macOS `sips` before sending. A startup **preflight** verifies Ollama is reachable and the model is pulled, printing the exact `ollama pull <model>` fix otherwise. **Resumable** — a song whose `.md` already exists is skipped (re-run any time; `--force` to redo, `--limit`/`--start`/`--ids` to scope). `data/songs-md/` is **gitignored** (like `images/`).
...
**Fix workflow** — a bad sheet is now a one-song re-run: `node scripts/local-backfill.mjs --ids <id> --force` (or tweak the `SYSTEM_INSTRUCTION` prompt in `local-backfill.mjs` for a systemic improvement), then `npm run chordpro:upload` + `node scripts/build-data.mjs` (or `npm run chordpro:fix -- <id>` to do all three in one shot). There is no OCR-rule layer, no per-song override JSON, no `chordpro:check` flagging — all of that (and the legacy EasyOCR/tesseract path, the `cp`/`flag` payload fields, the owner image-toggle, `ChordOverlay`, `sampleChordpro`) was removed earlier in the ChordPro migration.

> **Migration note:** the legacy local-OCR pipeline (EasyOCR/tesseract, `extract_chordpro.py`, `backfill.py`, `vlm_chordpro.py`, `regen.mjs`, `data/chordpro*`, `chordVocab.ts`, `chordOCR.ts`) is gone, and so is the interim **Gemini cloud** pipeline (`@google/genai`, `gemini-backfill.mjs`, `GEMINI_API_KEY`, the rate gate + circuit breaker). Extraction is now a **local Ollama vision-model call** ([scripts/local-backfill.mjs](scripts/local-backfill.mjs)) — on-device, keyless, unmetered.
